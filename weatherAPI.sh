...
# create the tooltip
TOOLTIP="$gTEMP$gTEMP_SUFFIX   &   $CONDITION_TEXT\r\rFeels Like:\t\t\t$gFEELSLIKE$gTEMP_SUFFIX\r\r"
if [ $COLD -eq 0 ]; then 
    if [ ! -z $gHEATINDEX ]; then 
        if [ $gHEATINDEX -gt $gTEMP ] && [ $gHEATINDEX -gt $gFEELSLIKE ]; then
            TOOLTIP+="Heat Index:\t\t$gHEATINDEX$gTEMP_SUFFIX\r\r"
        fi
    fi
else
    if [ ! -z $gWIND_CHILL ]; then
        if [ $gWIND_CHILL -lt $gTEMP ] && [ $gWIND_CHILL -lt $gFEELSLIKE ]; then
            TOOLTIP+="Wind Chill:\t\t$gWINDCHILL$gTEMP_SUFFIX\r\r"
        fi
    fi
fi
TOOLTIP+="Humidity:\t\t\t$HUMIDITY%\r"
TOOLTIP+="Pressure:\t\t\t$gPRESSURE $gPRESSURE_SUFFIX\r"
TOOLTIP+="UV:\t\t\t\t$UV ( $UVSTR )\r\r"
TOOLTIP+="Clouds:\t\t\t$CLOUD%\r"
TOOLTIP+="Wind:\t\t\t\t$gWIND $gWIND_SUFFIX from the $WIND_DIR\r"
TOOLTIP+="Gusting:\t\t\t$gGUST $gWIND_SUFFIX\r\r"
TOOLTIP+="Precipitation:\t\t${gFTOTALPRECIP[0]} $gPRECIP_SUFFIX expected ( ${FDAILY_CHANCE_OF_RAIN[0]}% probability )\r"
if [ $COLD -eq 1 ]; then
    TOOLTIP+="Snow:\t\t\t\t${FTOTALSNOW_CM[0]} cm\r\r"
fi
TOOLTIP+="Sunrise/set:\t\t${gFASTRO_SUNRISE[0]} / ${gFASTRO_SUNSET[0]}\r"
TOOLTIP+="Moonphase:\t\t$(echo -e \\U$SYMBOL) ${FASTRO_MOONPHASE[0]} ( ${FASTRO_MOON_ILLUMINATION[0]}% illuminated )\r\r"
TOOLTIP+="Today:\t\t\t\t${FCONDITION_TEXT[0]}, high: ${gFMAXTEMP[0]} low: ${gFMINTEMP[0]}\r"
TOOLTIP+="Tomorrow:\t\t${FCONDITION_TEXT[1]}, high: ${gFMAXTEMP[1]}, low: ${gFMINTEMP[1]}\r"
TOOLTIP+="Next Day:\t\t\t${FCONDITION_TEXT[2]}, high: ${gFMAXTEMP[2]}, low: ${gFMINTEMP[2]}\r\rLast updated: $LAST_UPDATED"

# send the notification (replace contents of existing notification if one exists)
NOTIF=$(cat /tmp/.notif)
[[ -z $NOTIF ]] && NOTIF=1
